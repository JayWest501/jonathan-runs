// /api/photos
// GET: returns public gallery photos from Supabase Storage
// POST: uploads gallery or gear photos (admin only, multipart form data)
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    let photos = data
      ...
      .filter(file => {
        const isGear = file.name.startsWith('gear__');
        return type === 'gear' ? isGear : !isGear;
      })
      .map(file => {
        const nameWithoutGearPrefix = file.name.startsWith('gear__')
          ? file.name.slice('gear__'.length)
          : file.name;

        const parts = nameWithoutGearPrefix.split('__');
        ...
        return {
          ...
          type: file.name.startsWith('gear__') ? 'gear' : 'gallery'
        };
      });

    if (race_id) {
      photos = photos.filter(photo => photo.race_id === race_id);
    }
  ...
  if (req.method === 'POST') {
    ...
    const raceIdPart = parts.find(part => part.name === 'race_id');
    const uploadTypePart = parts.find(part => part.name === 'upload_type');
    ...
    const caption = captionPart
      ? captionPart.data.toString('utf8').trim()
      : '';
    const raceId = raceIdPart
      ? raceIdPart.data.toString('utf8').trim()
      : '';
    const uploadType = uploadTypePart
      ? uploadTypePart.data.toString('utf8').trim()
      : 'gallery';
    ...
    const baseName = raceId
      ...
      : `${Date.now()}-${safeName}`;
    const fileName = uploadType === 'gear'
      ? `gear__${baseName}`
      : baseName;
    ...
    const { error: uploadError } = await supabase.storage
      ...
      .upload(fileName, filePart.data, {
        ...
        upsert: false,
        metadata: { caption, upload_type: uploadType }
      });
